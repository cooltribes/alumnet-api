class User < ActiveRecord::Base
  ...
  def groups_posts(q)
    ...
    Post.with_includes.joins(:postable_group).where(postable_type: "Group")
      ...
      .where('posts.updated_at > ?', 1.month.ago)
      .ransack(q).result
  end

  def my_likes_posts(q)
    posts = posts_by_like.with_includes.where('posts.updated_at > ?', 1.month.ago).ransack(q).result.to_a
  ...
  def my_posts(query)
    posts.with_includes.where('updated_at > ?', 1.month.ago).ransack(query).result | publications.with_includes.where('updated_at > ?', 1.month.ago).ransack(query).result
  end
