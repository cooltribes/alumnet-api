class UserMailer < ActionMailer::Base
  ...
  def user_request_approval(approver, requester)
    @approver = approver
    @requester = requester
    mail to: @approver.email, subject: "Is #{requester.name} an AIESEC Alum you know?"
  end
  ...
  def user_commented_post_you_commented_or_liked(user, comment)
    @user = user
    @comment = comment
  ...
  def group_digest(membership, posts)
    ...
    mail to: @membership.user.email, subject: "Alumnet digest: #{@membership.group.name}"
  end
end
