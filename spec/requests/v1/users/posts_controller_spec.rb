...
describe V1::Users::PostsController, type: :request do
  ...
  describe "GET /users/:user_id/posts/:id" do
    it "return a post of a user by id" do
      ...
      get user_post_path(user, post), {}, basic_header(author.api_token)
      expect(response.status).to eq 200
  ...
  describe "POST /users/:user_id/posts" do
    context "with valid attributes" do
      it "create a post in user" do
        expect {
          post user_posts_path(user), valid_attributes , basic_header(author.api_token)
    ...
    context "with invalid attributes" do
      it "return the errors in format json" do
        expect {
          post user_posts_path(user), invalid_attributes, basic_header(author.api_token)
  ...
  describe "PUT /users/:user_id/posts/:id" do
    it "edit a post of user" do
      post = Post.make!(postable: user)
      put user_post_path(user, post), { body: "New body of post" }, basic_header(author.api_token)
      expect(response.status).to eq 200
      post.reload
  ...
  describe "DELETE /users/:user_id/posts/:id" do
    it "delete a post of user" do
      post = Post.make!(postable: user)
      expect {
        delete user_post_path(user, post), {}, basic_header(author.api_token)
      }.to change(Post, :count).by(-1)
