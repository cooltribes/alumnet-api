  ...
  class SuggesterUsers
    ...
    private
      ...
      def get_by_committees
        ...
        @results[:by_committee] = filter_users_collection(users_arrays.flatten.uniq)
      end
      ...
      def get_by_residence
        ...
        @results[:by_residence_city] = filter_users_collection(users_in_city.to_a)
        users_in_country =  User.active.joins(:profile).where(profiles: { residence_country_id: residence_country })
          .where.not(id: user.id).uniq
        @results[:by_residence_country] = filter_users_collection(users_in_country.to_a)
      end

      def get_by_birth
        users_in_city = User.active.joins(:profile).where(profiles: { birth_city_id: birth_city })
          .where.not(id: user.id).uniq
        @results[:by_birth_city] = filter_users_collection(users_in_city.to_a)
        users_in_country =  User.active.joins(:profile).where(profiles: { birth_country_id: birth_country })
          .where.not(id: user.id).uniq
        @results[:by_birth_country] = filter_users_collection(users_in_country.to_a)
      end
